...
**Advisor Guidance:** To enroll in ${a.code} in a subsequent semester, register for ${m.join(", ")} first. Would you like me to show alternative electives you are currently qualified for?`}return{message:y,intent:"CHECK_ELIGIBILITY",steps:s,data:{course:a,isEligible:p,missingPrereqs:m,seats:r.available},requiresConfirmation:C,confirmationAction:n,suggestedQuickActions:p?[`Register for ${a.code}`,"Check Timetable","View Syllabus"]:["Find Eligible Alternatives","View Degree Roadmap","Check Prerequisites"]}}async handleRegistrationIntentFlow(e,t,i,s){let n=i[0]||this.extractCourse(e)||d.E.courses.get("CS401"),a=n.sections[0];if((t.currentEnrollments||[]).includes(n.code)||(t.completedCourses||[]).includes(n.code))return{message:`ℹ️ You are already enrolled in **${n.code} — ${n.name}**!

Under university policy, one course enrollment is permitted per student, and all lecture videos, lab environments, and completion certificates remain accessible to you lifelong.

You can access your learning curriculum and labs anytime under **Enrolled Courses**!`,intent:"ALREADY_ENROLLED",steps:s,suggestedQuickActions:["Open Course Learning","View Degree Progress","Explore Other Courses"]};let r=new Set(t.completedCourses||[]),o=(n.prerequisites||[]).filter(e=>!r.has(e));if(o.length>0)return{message:`❌ Cannot proceed with registration for **${n.code}**. You have missing prerequisite(s): **${o.join(", ")}**.
