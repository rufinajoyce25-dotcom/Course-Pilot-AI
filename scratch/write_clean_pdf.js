...
import { Student, Course, CourseCertificate } from "@/types";

/**
 * Generates an official Proof of Enrollment & Academic Confirmation as a downloadable PDF document.
 * Reflects full tuition-free status ($0.00) and lifelong access privileges.
 */
export function generateEnrollmentBillPdf(
  ...
  const sec = sectionCode
    ? course.sections.find((s) => s.sectionCode === sectionCode) || course.sections[0]
    : course.sections[0];
...
export function generateCertificatePdf(cert: CourseCertificate): void {
  ...
  // Gold Star Badge Icon placeholder
  ...
  doc.text(\`Final Assessment Score: \${cert.quizScore ?? 18} / 20 Marks (Passed • Passing Criterion: 12/20 Satisfied)\`, 148.5, 131, { align: "center" });
...
fs.writeFileSync('./src/lib/utils/pdfGenerator.ts', pdfCode, 'utf8');
console.log('Clean pdfGenerator.ts written successfully');
